...
export class Monomial {
  ...
  constructor(
    ...
    exp: Float64Array = Float64Array.from([0, 0, 0, 0]),
    vars: string[] = ["t", "x", "y", "z"]
  ) {
  ...
  pushVariables(newVars: string[]) {
    ...
    this.exp = Float64Array.from(
      Array.from(this.exp).concat(varsToAdd.map((v) => 0))
    );
  }
  ...
  removeVariables(oldVars: string[]) {
    ...
    let newExp = this.exp.filter(
      (e, idx) => !varsToRemove.includes(this.vars[idx])
    );
    this.vars = newVars;
    this.exp = newExp;
  }
  ...
  setCoef(coef: Fraction | number) {
    if (typeof coef === "number") this.coef = new Fraction(coef, 1);
    else this.coef = coef;
  }
   ...
   */
  sameVars(m: Monomial) {
    if (this.vars.length !== m.getVars().length) return false;
  ...
  gcd(m: Monomial) {
    if (!this.sameVars(m)) {
      throw new Error(
        "CAN NOT DETERMINE GCD OF MONOMIALS USING DIFFERENT VARIABLES"
      );
    }
  ...
  lcm(m: Monomial) {
    if (!this.sameVars(m)) {
      throw new Error(
        "CAN NOT DETERMINE LCM OF MONOMIALS USING DIFFERENT VARIABLES"
      );
    }
  ...
  plus(m: Monomial) {
    if (!m.equalExponent(this) || !m.sameVars(this))
      throw new Error(
        `TRYING TO SUM MONOMIALS WITH DIFFERENT EXPONENT OR VARIABLES`
      );
  ...
  multiply(m: Monomial | Fraction | number) {
    ...
    if (m instanceof Fraction) {
      ...
      if (!m.sameVars(this))
        throw new Error(
          "TRYING TO MULTIPLY MONOMIALS WITH DIFFERENT EXPONENT OR VARIABLES"
        );
  ...
  divide(m: Monomial | Fraction | number) {
    ...
    if (m instanceof Fraction) {
      if (m.isZero()) throw new Error("TRYING TO DIVIDE MONOMIAL BY 0");

      return new Monomial(this.coef.divide(m), this.exp, this.vars);
    } else {
      if (!m.sameVars(this))
        throw new Error(
          "TRYING TO DIVIDE MONOMIALS WITH DIFFERENT EXPONENT OR VARIABLES"
        );
   ...
   */
  le(m: Monomial) {
    if (!this.sameVars(m)) throw new Error("Monomials in different rings");
  ...
  toString(showProductChar: boolean = false) {
    ...
    if (this.coef.toNumber() < 0)
      mon = [mon.slice(0, 1), " ", mon.slice(1)].join("");

    let added = 0;
    let varsPresent = this.vars.filter(
      (val, idx) => this.exp[idx] !== 0
    ).length;
    ...
    this.vars.forEach((v, idx) => {
      ...
      if (this.exp[idx] !== 0) {
        mon += `${
          added > 0 && added < varsPresent && showProductChar ? "*" : ""
        }${v}${e !== 0 && e !== 1 ? `^${this.exp[idx]}` : ""}`;
        added++;
      }
    });
