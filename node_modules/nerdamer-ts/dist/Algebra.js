...
(function () {
    ...
    Polynomial.prototype = {
        ...
        multiply: function (poly) {
            var l1 = this.coeffs.length, l2 = poly.coeffs.length, c = []; //array to be returned
            for (var i = 0; i < l1; i++) {
                var x1 = this.coeffs[i];
                for (var j = 0; j < l2; j++) {
                    var k = i + j, //add the powers together
                    x2 = poly.coeffs[j], e = c[k] || new Frac(0); //get the existing term from the new array
    ...
    var __ = core.Algebra = {
        ...
        proots: function (symbol, decp) {
            ...
            if (symbol instanceof Symbol && symbol.isPoly()) {
                ...
                var variable = keys(symbol.symbols).sort().pop(), sym = symbol.group === core.groups.PL ? symbol.symbols : symbol.symbols[variable], g = sym.group, powers = g === S ? [sym.power.toDecimal()] : keys(sym.symbols), rarr = [], max = core.Utils.arrayMax(powers); //maximum power and degree of polynomial to be solved
                // Prepare the data
                for (var i = 1; i <= max; i++) {
            ...
            function calcroots(rarr, powers, max) {
                ...
                function QuadIT_ak1(DBL_EPSILON, N, iPar, uu, vv, qp, NN, sdPar, p, qk, calcPar, K) {
                    ...
                    var qPar = new Object(), ee, mp, omp, relstp, t, u, ui, v, vi, zm, i, j = 0, tFlag, triedFlag = 0; // Integer variables
                ...
                function RealIT_ak1(DBL_EPSILON, iPar, sdPar, N, p, NN, qp, K, qk) {
                    ...
                    var ee, kv, mp, ms, omp, pv, s, t, dumFlag, i, j, nm1 = N - 1; // Integer variables
                    iPar.NZ = j = dumFlag = 0;
                    s = sdPar.a;
                    for (;;) {
                        pv = p[0];
                ...
                function Fxshfr_ak1(DBL_EPSILON, MDP1, L2, sr, v, K, N, p, NN, qp, u, iPar) {
                    ...
                    qk = new Array(MDP1), svk = new Array(MDP1), a, b, betas, betav, oss, ots, otv, ovv, s, ss, ts, tss, tv, tvv, ui, vi, vv, fflag, i, iFlag = 1, j, spass, stry, tFlag, vpass, vtry; // Integer variables
                    iPar.NZ = 0;
                    betav = betas = 0.25;
                    oss = sr;
                    ovv = v;
                ...
                function rpSolve(degPar, p, zeror, zeroi) {
                    var N = degPar.Degree, RADFAC = 3.14159265358979323846 / 180, // Degrees-to-radians conversion factor = PI/180
                    LB2 = Math.LN2, // Dummy variable to avoid re-calculating this value in loop below
                    MDP1 = degPar.Degree + 1, K = new Array(MDP1), pt = new Array(MDP1), qp = new Array(MDP1), temp = new Array(MDP1), 
                    ...
                    Fxshfr_Par = new Object(), bnd, DBL_EPSILON, df, dx, factor, ff, moduli_max, moduli_min, sc, x, xm, aa, bb, cc, sr, t, u, xxx, j, jj, l, NM1, NN, zerok; // Integer variables
                    ...
                    var LO = Number.MIN_VALUE / DBL_EPSILON, cosr = Math.cos(94.0 * RADFAC), // = -0.069756474
        ...
        froot: function (f, guess, dx) {
            var newtonraph = function (xn) {
                ...
                fn = f instanceof Function ? f : core.Utils.build(f), max = 10000, done = false, safety = 0;
                while (!done) {
        ...
        Factor: {
            ...
            mfactor: function (symbol, factors) {
                if (symbol.group === FN) {
                    ...
                    var vars = variables(symbol), symbols = symbol.collectSymbols().map(function (x) {
            ...
            try {
                ...
                if (vars.length === 1) {
                    ...
                    var get_unique_max = function (term, any) {
                        var max = Math.max.apply(null, term.terms), count = 0, idx;
                        if (!any) {
                    ...
                    var get_det = function (s, lookat) {
                        ...
                        for (var i = lookat + 1; i < l; i++) {
                            var term = s[i], is_equal = det.sum.equals(term.sum);
                            if (!is_equal && umax) {
                                break;
                            }
                            if (is_equal) {
                                ...
                                var d1 = max1.subtract(term.terms[idx1]), d2 = max2.subtract(det.terms[idx2]);
                                if (d2 > d1) {
                                    umax = [max2, idx2, term];
                                    break;
                                }
