...
(function () {
    ...
    var __ = core.Solve = {
        ...
        getPoints: function (symbol, step, points) {
            ...
            var f = build(symbol);
            var x0 = 0;
            var start = Math.round(x0), last = f(start), last_sign = last / Math.abs(last), rside = core.Settings.ROOTS_PER_SIDE, // the max number of roots on right side
    ...
    var solve = function (eqns, solve_for, solutions, depth, fn) {
        ...
        var eq = (core.Utils.isSymbol(eqns) ? eqns : __.toLHS(eqns, false)).getNum(), vars = core.Utils.variables(eq), //get a list of all the variables
        ...
        var separate = function (eq) {
            var lhs = new Symbol(0), rhs = new Symbol(0);
            eq.each(function (x) {
            ...
            if (!eq.hasFunc(solve_for) && eq.isComposite()) {
                try {
                    ...
                    if (factored.group === CB) {
                        ...
                        var l = coeffs.length, deg = l - 1; //the degree of the polynomial
