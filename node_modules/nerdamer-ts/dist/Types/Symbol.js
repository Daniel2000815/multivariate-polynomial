...
class Symbol {
    ...
    getDenom() {
        ...
        symbol = this.clone();
        //e.g. 1/(x*(x+1))
        if (this.group === Groups_1.Groups.CB && this.power.lessThan(0))
            symbol = (0, expand_1.expand)(symbol);
        //if the symbol already is the denominator... DONE!!!
        if (symbol.power.lessThan(0)) {
            let d = (0, Parser_1.parse)(symbol.multiplier.den);
            ...
            for (let x in symbol.symbols)
                if (symbol.symbols[x].power < 0)
                    retval = (0, Core_1.multiply)(retval, symbol.symbols[x].clone().invert());
        }
        else
            retval = (0, Parser_1.parse)(symbol.multiplier.den);
        return retval;
    ...
    getNum() {
        ...
        symbol = this.clone();
        //e.g. 1/(x*(x+1))
        if (symbol.group === Groups_1.Groups.CB && symbol.power.lessThan(0))
            symbol = (0, expand_1.expand)(symbol);
        //if the symbol already is the denominator... DONE!!!
    ...
    variables(poly = false, vars = undefined) {
        ...
        if (group === Groups_1.Groups.CP || group === Groups_1.Groups.CB || prevgroup === Groups_1.Groups.CP || prevgroup === Groups_1.Groups.CB) {
            ...
            let fo = (0, Utils_1.firstObject)(this.symbols);
            if ((0, Utils_1.isSymbol)(fo))
                fo.variables(poly, vars);
        }
        else if (group === Groups_1.Groups.EX) {
            if (!isNaN(this.value)) {
                vars.add(this.value);
            ...
            for (let i = 0; i < this.args.length; i++) {
                if ((0, Utils_1.isSymbol)(this.args[i]))
                    this.args[i].variables(poly, vars);
            }
        }
        return vars.c.sort();
    }
