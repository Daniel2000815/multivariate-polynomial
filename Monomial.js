...
export class Monomial {
     ...
     */
    constructor(coef = 0, exp = Float64Array.from([0, 0, 0, 0]), vars = ["t", "x", "y", "z"]) {
     ...
     */
    equalExponent(m) {
        return (this.exp.length === m.exp.length &&
    ...
    toString(showProductChar = false) {
        ...
        if (this.isOne())
            return "1";
        let mon = "";
        if (this.coef === -1 && this.exp.some(e => e !== 0))
            mon = "-";
        else if (this.coef === 1 && !this.exp.every(e => e === 0))
            mon = "";
        else
            mon = this.coef.toString();
