...
const divisionTests = [
  ...
  { f: "0", fs: ["4*x", "5"] },
  {
    f: "6*x*t^2 + 5*x + 5*y*z^6",
    fs: ["3*x^4 + 5*y*t + z^2", "t^2 + x - 1", "3*x + 6*t^2", "x^2"],
  },
];
...
describe("Security", function () {
  ...
  it("Division by 0 throws error", function () {
    assert.throws(() =>
      new Polynomial("2x").divide([new Polynomial("2*y"), new Polynomial("0")])
    );
  });
});
  ...
  for (var i = 0; i < expGreaterTests.length; i++) {
    (function (i) {
      ...
      it(`${t.e1} ${t.res ? ">" : "<="} ${t.e2}`, function () {
        if (t.res !== null)
          assert.equal(
            Polynomial.expGreater(
              Float64Array.from(t.e1),
              Float64Array.from(t.e2)
            ),
            t.res
          );
        else
  ...
  for (var i = 0; i < leaderTests.length; i++) {
    (function (i) {
      ...
      it(`p=${t.p} => lc=${t.lc}, lm=${t.lm}`, function () {
        const p = new Polynomial(t.p);
        assert.equal(
          p.lc().equals(new Fraction(t.lc)) &&
            p.lm().equals(new Monomial(1, Float64Array.from(t.lm))),
          true
        );
      });
    })(i);
  ...
  for (var i = 0; i < variablesTest.length; i++) {
    (function (i) {
      ...
      it(`${t.p} INSERT ${t.insertVars} AT POSITION ${t.pos} (SUPPORT)`, function () {
        assert.deepEqual(
          p.supp(),
          t.supp.map((r) => Float64Array.from(r))
        );
      });
    })(i);
  }
  ...
  for (var i = 0; i < suppTests.length; i++) {
    (function (i) {
      ...
      it(`supp(${t.p}) = ${t.res}`, function () {
        assert.deepEqual(
          new Polynomial(t.p).supp(),
          t.res.map((r) => Float64Array.from(r))
        );
      });
    })(i);
  }
