...
const zero = new Monomial(0, Float64Array.from([0, 0, 0, 0]), [
  "t",
  "x",
  "y",
  "z",
]);
const one = new Monomial(1, Float64Array.from([0, 0, 0, 0]), [
  "t",
  "x",
  "y",
  "z",
...
describe("Security", function () {
  ...
  it("Initialized only with coef returns that coef", function () {
    ...
    assert(m.plus(new Monomial(-2)).equals(zero));
  });
  ...
  it("Initialized with wrong variables length returns error", function () {
    const m = new Monomial(2);
    assert.throws(
      () => new Monomial(2, Float64Array.from([1, 2, 3, 4]), ["t", "x"])
    );
  });
...
describe("Same vars", function () {
  for (var i = 0; i < sameVarsTest.length; i++) {
    (function (i) {
      ...
      it(`${testMonomials[t.i].getVars()} ${t.res ? "=" : "!="} ${testMonomials[
        t.j
      ].getVars()}`, function () {
...
describe("Add vars", function () {
  for (var i = 0; i < addVarsTest.length; i++) {
    (function (i) {
      ...
      const varsToRemove = [...new Set(t.added)].filter((v) =>
        t.old.includes(v)
      );
...
describe("Comparisons", function () {
  for (var i = 0; i < compTests.length; i++) {
    (function (i) {
      ...
      it(`${testMonomials[t.i].toString()} ${
        t.le ? "==" : "!="
      } ${testMonomials[t.j].toString()}`, function () {
...
describe("GCD", function () {
  for (var i = 0; i < gcdLcmTests.length; i++) {
    (function (i) {
      ...
      it(`gcd(${testMonomials[t.i].toString()}, ${testMonomials[
        t.j
      ].toString()}) = ${t.gcd ? t.gcd : "-"}`, function () {
        if (t.gcd)
          assert.equal(
            testMonomials[t.i].gcd(testMonomials[t.j]).toString(),
            t.gcd
          );
        else assert.throws(() => testMonomials[t.i].gcd(testMonomials[t.j]));
      });
    })(i);
  }
});
...
describe("LCM", function () {
  for (var i = 0; i < gcdLcmTests.length; i++) {
    (function (i) {
      ...
      it(`lcm(${testMonomials[t.i].toString()}, ${testMonomials[
        t.j
      ].toString()}) = ${t.lcm ? t.lcm : "-"}`, function () {
        if (t.gcd)
          assert.equal(
            testMonomials[t.i].lcm(testMonomials[t.j]).toString(),
            t.lcm
          );
        else assert.throws(() => testMonomials[t.i].lcm(testMonomials[t.j]));
      });
    })(i);
  }
});
